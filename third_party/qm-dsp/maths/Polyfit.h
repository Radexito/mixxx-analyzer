...
class TPolyFit {
  ...
  public:
    static double PolyFit2(const vector<double> &x,  // does the work
                           const vector<double> &y, vector<double> &coef);
  ...
  private:
    ...
    static bool GaussJordan2(Matrix &b, const vector<double> &y, Matrix &w,
...
double TPolyFit::PolyFit2(const vector<double> &x, const vector<double> &y, vector<double> &coefs)
// nterms = coefs.size()
// npoints = x.size()
{
    int i, j;
...
void TPolyFit::Square(const Matrix &x, const vector<double> &y, Matrix &a, vector<double> &g,
                      const int nrow, const int ncol) {
    int i, k, l;
    for (k = 0; k < ncol; ++k) {
        for (l = 0; l < k + 1; ++l) {
...
bool TPolyFit::GaussJordan(Matrix &b, const vector<double> &y, vector<double> &coef)
...
bool TPolyFit::GaussJordan2(Matrix &b, const vector<double> &y, Matrix &w,
                            vector<vector<int> > &index) {
    ...
    for (int i = 0; i < ncol; ++i) {
        ...
        for (int j = 0; j < ncol; ++j) {
            if (index[j][2] != 0) {
                for (int k = 0; k < ncol; ++k) {
                    if (index[k][2] > 0) {
                        std::cerr << "ERROR: Error in PolyFit::GaussJordan2: matrix is singular"
                                  << std::endl;
                        return false;
                    }
