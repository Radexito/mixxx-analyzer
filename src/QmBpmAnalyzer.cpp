...
std::vector<ConstRegion> retrieveConstRegions(const std::vector<double>& beatFrames,
                                              int sampleRate) {
    if ((int)beatFrames.size() < 2)
        return {};
    ...
    while (leftIndex < (int)beatFrames.size() - 1) {
        double meanBeatLength =
            (beatFrames[rightIndex] - beatFrames[leftIndex]) / (rightIndex - leftIndex);
        int outliersCount = 0;
...
std::optional<double> trySnap(double minBpm, double center, double maxBpm, double fraction) {
    ...
    if (snap > minBpm && snap < maxBpm)
        return snap;
    return std::nullopt;
}

double roundBpmWithinRange(double minBpm, double center, double maxBpm) {
    if (auto s = trySnap(minBpm, center, maxBpm, 1.0))
        return *s;
    if (center < 85.0) {
        if (auto s = trySnap(minBpm, center, maxBpm, 2.0))
            return *s;
    }
    if (center > 127.0) {
        if (auto s = trySnap(minBpm, center, maxBpm, 2.0 / 3.0))
            return *s;
    }
    if (auto s = trySnap(minBpm, center, maxBpm, 3.0))
        return *s;
    if (auto s = trySnap(minBpm, center, maxBpm, 12.0))
        return *s;
    return center;
}
...
double makeConstBpm(const std::vector<ConstRegion>& regions, int sampleRate) {
    ...
    for (int i = 0; i < midRegionIndex; ++i) {
        ...
        const int numberOfBeats = static_cast<int>(length / regions[i].beatLength + 0.5);
        if (numberOfBeats < kMinRegionBeatCount)
            continue;

        const double thisMin =
            regions[i].beatLength - (kMaxSecsPhaseError * sampleRate) / numberOfBeats;
        const double thisMax =
            regions[i].beatLength + (kMaxSecsPhaseError * sampleRate) / numberOfBeats;
        ...
        const double newLength = regions[midRegionIndex + 1].firstBeat - regions[i].firstBeat;
    ...
    for (int i = (int)regions.size() - 2; i > midRegionIndex; --i) {
        ...
        const double newLength = regions[i + 1].firstBeat - regions[startRegionIndex].firstBeat;
