...
TempoTrackV2::TempoTrackV2(float rate, int increment) : m_rate(rate), m_increment(increment) {}
...
// as it was before
void TempoTrackV2::calculateBeatPeriod(const vector<double> &df, vector<int> &beat_period,
    ...
    // this might (will?) break if a user specifies a different frame rate for the onset detection
    // function
    ...
    if (constraintempo) {
        ...
        for (int i = 0; i < wv_len; i++) {
            ...
            wv[i] = (double(i) / pow(rayparam, 2.)) *
                    exp((-1. * pow(-double(i), 2.)) / (2. * pow(rayparam, 2.)));
        }
    }
...
void TempoTrackV2::calculateBeats(const vector<double> &df, const vector<int> &beat_period,
    ...
    while (backlink[ibeats.back()] > 0) {
        //        std::cerr << "backlink[" << ibeats.back() << "] = " << backlink[ibeats.back()] <<
        //        std::endl;
        int b = ibeats.back();
        if (backlink[b] == b)
